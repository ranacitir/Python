    ...
    if (gameOver == False):
        ...
        if not (steve.isJump):
            ...
            if steve.jumpCount >= -15:
                steve1.rect.y -= (steve.jumpCount * steve.jj) * 0.5  # ZIPLAMA HIZINI VE YÜKLEKLİĞİNİ AYARLAR
                steve.jumpCount -= 1
            else:
                steve.isJump = False
                steve.jumpCount = 15

    if (pygame.sprite.collide_mask(toz2, steve1) or pygame.sprite.collide_mask(fire2, steve1) or
            pygame.sprite.collide_mask(cactus1, steve1) or pygame.sprite.collide_mask(cactus2, steve1)):
        gameOver = True
    else:
        gameOver = False
